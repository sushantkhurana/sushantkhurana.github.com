	...
	validateCall : function(caller,rules) {	// EXECUTE VALIDATION REQUIRED BY THE USER FOR THIS FIELD
		...
		for (i=0; i<rules.length;i++)
		{
			...
			case "regexcheck":
				...
				//alert("in2:"+customRule);
				if(r == 1 && customRule != "" && ($(caller).attr('value') == "" || $(caller).attr('value') != ""))
				{
					_regex_val(caller,customRule,i);
					
				}
				else if(r == 0 && customRule != "" && $(caller).attr('value') != "")
				{
					_regex_val(caller,customRule,i);
				}
				///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		//My code////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		function _regex_val(caller,rules,position)
		{
			// VALIDATE REGEX RULES
			var customRule;
			var email_check="@";
			...
			if(rules.charAt(0)=="#"){
				customRule=rules.replace(/#/g,"/");
			}
			else{
				customRule=rules;
			}
			
			var res=customRule.search(email_check);
			
			pattern = eval(customRule);
			//call for MultipleEmailsCommaSeparated validation
			if(res != -1 && $(caller).attr('value').search(",") != -1){
				validateMultipleEmailsCommaSeparated( $(caller).attr('value') , pattern );
			}
			else{//general validation
				//regex= (res != -1) ? eval($.validationEngine.settings.allrules["email"].regex) : eval(customRule);
				regex= eval(customRule);

				if(!regex.test($(caller).attr('value'))){
	    			$.validationEngine.isError = true;
	    			//alert("error");
	    			error_flag=1;
	    		}
		...
		function validateMultipleEmailsCommaSeparated( value , regex ){
		...
		function _customRegex_val(caller,rules,position)
		{		
			var customRule="";
			...
			if(error_flag==1 && customRule!="")
			{
				promptText += $.validationEngine.settings.allrules['custom_val'].alertText+customRule+"<br />";
			}
		}
		...
		function _length1(caller,rules,position)
		{    	  // VALIDATE Textbox LENGTH
			...
			if(feildLength<startLength || feildLength>endLength){
				$.validationEngine.isError = true;
				promptText += $.validationEngine.settings.allrules["length1"].alertText+startLength+$.validationEngine.settings.allrules["length1"].alertText2+endLength+$.validationEngine.settings.allrules["length1"].alertText3+"<br />"
			}
		}
